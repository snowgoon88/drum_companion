* __CURRENT_: display save/read/(edit) loops

* GitVersion:
** imgui working: 2c03aac6d (v 1.87)
** miniaudio: 82e70f4 (v 0.11.8)
* PlayLoop
l'idée est de faire un lecteur "Live", avec des boucles, pour utiliser avec Marie.
** lire MiniAudio/PCM pour comprendre si on peut "looper" de manière "intrinsèque"
* [2/2] loop (4 - Data Sources)
** DONE essayer de faire une loop => 11-simple-innerloop.cpp
** [6/6] display wav in ImGUI
*** DONE prevent from zooming in Y
*** DONE restict zooming in X (max = size, min = ??)
*** DONE display in frames OR seconds
*** DONE restore to full view
*** DONE remove legende
*** DONE remove mouse position
** DONE [3/3] ImGUI ergonomy
*** DONE quit on Ctrl-Q
*** DONE full size
*** DONE out of the "Debug" sub_windo ?
by default, each Frame has a "Debug" window
** [3/3] play
*** DONE Play / Stop buttons
*** DONE Play-Pause button
*** DONE change color of button when playing ?
*** toStart button
*** [1/2] sliding line synchronized
**** DONE red vert line at pCursor
**** TODO sliding ??
** [1/3] loop enabled
*** DONE with ONE loop
*** TODO save/read loop (and song) in yaml file ?
*** TODO edit loop => Edit mode and 'M'arker, 'S' start from here ?
*** several loops ??
** [4/4] Keyboard shortcuts
*** DONE play/pause : SPACE
*** DONE stop :       Ctrl-SPACE
*** DONE loop :       ENTER
*** DONE hint or textline about Key shorcuts
** several wavs
* IDEA [28/34] linux
...
* Playback
** iAudio7
from https://www.notice-facile.com/notice/20796/cowon+systems+iaudio-7-_f
Flac: compression 0~8, 44.1 kHz, mono/stereo
WAV: 48 kHz, 16 bit, mono/stereo
ogg: ~q10, 44.1kHz, mono/stereo
** convert
see https://www.maketecheasier.com/resample-hi-res-flac-linux/
for flac in *.flac; do sox -S "${flac}" resample/"${flac}" rate -v -L 44100 dither; done
** miniaudio
https://miniaud.io/docs/manual/index.html
*** Concepts

**** MiniAudio Objects
you need to allocate memory for objects yourself. => Attention au STACK
you must ensure the memory address of your objects remain the same throughout their lifetime. => NO COPY !!!
**** Low level API
donnée audio raw de l'audio device.
Attention, "loopback" ne semble possible qu'en WASAPI (??)
***** context: top level, permet d'énumérer les devices
***** device: abstraction pour physical device
move data to/from device, asynchronously.
***** frame: one sample for each channel
***** ma_decoder is decoupled from Engine and Device => read file
**** High level API
***** ressource management loading and streaming sounds
***** node graph pour mixing and effect
***** high level "engine" autour de ressource manager
*** [/] pas compris
ASIO et WASAPI sont des interfaces différentes que les applications peuvent utiliser pour lire du son sous Windows. WASAPI est l'interface native actuelle de Microsoft pour la lecture audio, et ASIO est une alternative tierce conçue pour répondre au marché audio professionnel il y a plusieurs décennies.
