...
/**
 * MiniAudio + ImGUI with Play/Pause/Stop Button
 */
...
int main(int, char **) {
  ...
  // load music
  char music_path[] = "ressources/recordaMe_220307-01.wav";
  
  std::cout << "__Loading " << std::endl;
  ma_sound music_sound;
  result = ma_sound_init_from_file( &engine, music_path, MA_SOUND_FLAG_DECODE,
                                    NULL, /* no group -> engine end point*/
                                    NULL, /* no loading fence */
                                    &music_sound );
  if (result != MA_SUCCESS) {
    std::cerr << "Failed to load " << music_path << std::endl;
    return result;
  }
  //ma_data_source* music_source =  ma_sound_get_data_source( &music_sound );
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // created a named window.
    {
      ...
      ImGui::Begin("Drum Companion (play/pause)");
      // Display some text (you can use a format strings too)
      ImGui::Text("Music : Recorda Me");
    ...
    // MiniAudio
    if (should_run) {
      if (ma_state == ready || ma_state == paused) {
        result = ma_sound_start( &music_sound );
        if (result != MA_SUCCESS) {
    ...
    else if (should_stop && (ma_state == running)) {
      result = ma_sound_stop( &music_sound );
      if (result != MA_SUCCESS) {
          std::cerr << "Failed to stop"<< std::endl;
          ma_engine_uninit( &engine );
          return result;
      }
      result = ma_sound_seek_to_pcm_frame( &music_sound, 0 ); // "rewind" sound
      if (result != MA_SUCCESS) {
          std::cerr << "Failed to rewind sound"<< std::endl;
          ma_engine_uninit( &engine );
          return result;
      }
    ...
    else if (should_pause  && (ma_state == running)) {
      result = ma_sound_stop( &music_sound );
      if (result != MA_SUCCESS) {
